...
// TestBasic ensures that there is a ready azure-cloud-node-manager-app daemonset.
func TestBasic(t *testing.T) {
	ctx := context.Background()
	var err error

	// Check azure-cloud-node-manager-app deamonset is ready.
	err = checkReadyDaemonset(ctx)
	if err != nil {
		t.Fatalf("could not get azure-cloud-node-manager-app: %v", err)
	}
}
...
func checkReadyDaemonset(ctx context.Context) error {
	...
	o := func() error {
		selector := fmt.Sprintf("%s=%s", "app", app)
